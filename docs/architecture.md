...
文件治理和监控共用控制平面，但不是同一条安全链路：文件治理会在审批后产生受控磁盘副作用；监控只对管理员预先登记的回环 HTTP 地址或 Windows 服务做只读探测。监控事件只能说明目标健康状态，不能授权动作，也不会自动重启服务或扫描整台电脑。
...
当前运行态和未完成验收不写在本架构文档中，以避免架构说明随每次本地启动变化；请查看根目录 `CONTEXT.md`、`TODO.md` 和最新执行报告。
