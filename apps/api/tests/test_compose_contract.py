...
def test_verify_script_separates_e2e_api_and_web_ports() -> None:
    ...
    assert 'e2e-port.ps1' in script
    assert 'Get-FreeLoopbackPortRange -StartPort 18230 -Count 3' in script
    assert 'Get-FreeLoopbackPortRange -StartPort 18300 -Count 3' in script
    assert '$env:AGENTGATE_E2E_API_PORT = [string]$e2eApiPort' in script
    assert '$env:AGENTGATE_E2E_WEB_PORT = [string]$e2eWebPort' in script
    assert '$env:AGENTGATE_E2E_API_PORT = "18000"' not in script


def test_e2e_server_filters_expected_windows_disconnects() -> None:
    server = (REPO_ROOT / "apps/api/e2e_server.py").read_text(encoding="utf-8")
    config = (REPO_ROOT / "apps/web/playwright.config.ts").read_text(encoding="utf-8")
    assert "ConnectionResetError" in server
    assert "set_exception_handler" in server
    assert "winerror" in server
    assert "uvicorn.Server" in server
    assert "e2e_server.py" in config
...
def test_web_server_sets_baseline_browser_security_headers() -> None:
    ...
    assert "add_header Content-Security-Policy \"default-src 'self';" in nginx
...
def test_python_runtime_locks_exist_for_api_and_worker() -> None:
    for relative_path in (
        ...
        assert "--no-index" in lock.splitlines()[4]


def test_api_image_and_windows_setup_use_dependency_locks() -> None:
...
def test_start_local_contract_exists_and_checks_all_required_services() -> None:
    contract_path = REPO_ROOT / "scripts" / "start-local.contract.test.ps1"
    ...
    assert "api.*?scheduler.*?control-worker.*?web" in contract


def test_worker_soak_contract_requires_fresh_continuous_evidence() -> None:
    contract_path = REPO_ROOT / "scripts" / "soak-worker.contract.test.ps1"
    assert contract_path.is_file()
    contract = contract_path.read_text(encoding="utf-8")
    for expected in (
        "soak-worker.ps1",
...
def test_backup_restore_scripts_have_explicit_safe_modes() -> None:
    backup = (REPO_ROOT / "scripts/backup-local.ps1").read_text(encoding="utf-8")
    restore = (REPO_ROOT / "scripts/restore-local.ps1").read_text(encoding="utf-8")
    contract = (REPO_ROOT / "scripts/backup-restore.contract.test.ps1").read_text(
        encoding="utf-8"
    )
    for expected in (
        "[Parameter(Mandatory",
        ...
        "Apply",
        "Offline",
        "CurrentStateBackupDestination",
        "System.IO.File",
    ):
        assert expected in backup + restore + contract
    assert "worker_journal" in restore
    assert "journal.db" in restore
