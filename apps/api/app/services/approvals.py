...
class ApprovalService:
    ...
    async def _decide(
        ...
        if not self.action_repository.transition(
            action_id, {ActionStatus.PENDING_APPROVAL}, target
        ):
        ...
        self.run_repository.save_checkpoint(action.run_id, messages, run.step_count)
        self.run_repository.set_status(
            action.run_id, {RunStatus.WAITING_APPROVAL}, RunStatus.RUNNING
        )
