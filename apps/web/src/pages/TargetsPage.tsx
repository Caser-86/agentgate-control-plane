...
import { api, localMonitorEndpoint } from "../api/client";
...
export function TargetsPage() {
  ...
  const configuredEndpoint = localMonitorEndpoint(apiBaseUrl);
  const [targets, setTargets] = useState<MonitorTarget[]>([]);
  const [form, setForm] = useState<CreateMonitorTargetRequest>({ ...initialForm, endpoint: configuredEndpoint });
  ...
  useEffect(() => {
    setForm((current) => current.endpoint === initialForm.endpoint
      ? { ...current, endpoint: configuredEndpoint }
      : current);
  }, [configuredEndpoint]);
  ...
  const loadTargets = () => {
    ...
    void api.listMonitorTargets()
      ...
      .catch((error) => setFeedback({
        kind: "error",
        message: error instanceof Error && error.message ? error.message : "无法加载监控目标。",
      }))
  ...
  async function createTarget(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm({ ...initialForm, endpoint: configuredEndpoint });
    } catch (error) {
      setFeedback({
        kind: "error",
        message: error instanceof Error && error.message
          ? error.message
          : "无法创建目标，请检查名称、地址和参数。只允许监控本机回环地址。",
      });
    } finally {
      setSaving(false);
    }
  ...
  async function probeTarget(target: MonitorTarget) {
    ...
    try {
      ...
      setFeedback({ kind: "success", message: "探测任务已排队，完成后请刷新查看结果。" });
    } catch (error) {
      setFeedback({
        kind: "error",
        message: error instanceof Error && error.message ? error.message : "无法排队探测任务。",
      });
    } finally {
      setProbing(null);
    }
      ...
      <section className="panel composer monitor-form-panel">
        <div className="section-heading">
          <div><span className="eyebrow">登记本机目标</span><h2>添加只读监控</h2></div>
           <span className="mono-note">回环地址 / 只读</span>
        </div>
        <form className="monitor-form" onSubmit={(event) => void createTarget(event)}>
          <label htmlFor="monitor-name">目标名称<input id="monitor-name" name="name" autoComplete="off" value={form.name} onChange={(event) => updateForm("name", event.target.value)} placeholder="例如：本地 API…" required /></label>
          <label htmlFor="monitor-kind">目标类型
            <select id="monitor-kind" name="kind" autoComplete="off" value={form.kind} onChange={(event) => {
              const kind = event.target.value as MonitorTargetKind;
              updateForm("kind", kind);
              updateForm("endpoint", kind === "http" ? configuredEndpoint : "AgentGateWorker");
            }}>
              <option value="http">HTTP 地址</option>
              <option value="windows_service">Windows 服务</option>
            </select>
          </label>
          <label htmlFor="monitor-endpoint">{form.kind === "http" ? "本机 HTTP 地址" : "Windows 服务名"}<input id="monitor-endpoint" name="endpoint" autoComplete="off" type={form.kind === "http" ? "url" : "text"} value={form.endpoint} onChange={(event) => updateForm("endpoint", event.target.value)} placeholder={form.kind === "http" ? `${configuredEndpoint}…` : "例如：AgentGateWorker…"} required /></label>
          <div className="monitor-form-grid">
            <label htmlFor="monitor-interval">间隔（秒）<input id="monitor-interval" name="interval_seconds" autoComplete="off" type="number" min={5} max={86400} value={form.interval_seconds} onChange={(event) => updateForm("interval_seconds", Number(event.target.value))} /></label>
            <label htmlFor="monitor-timeout">超时（秒）<input id="monitor-timeout" name="timeout_seconds" autoComplete="off" type="number" min={1} max={30} value={form.timeout_seconds} onChange={(event) => updateForm("timeout_seconds", Number(event.target.value))} /></label>
            <label htmlFor="monitor-failure-threshold">失败阈值<input id="monitor-failure-threshold" name="failure_threshold" autoComplete="off" type="number" min={1} max={10} value={form.failure_threshold} onChange={(event) => updateForm("failure_threshold", Number(event.target.value))} /></label>
            <label htmlFor="monitor-recovery-threshold">恢复阈值<input id="monitor-recovery-threshold" name="recovery_threshold" autoComplete="off" type="number" min={1} max={10} value={form.recovery_threshold} onChange={(event) => updateForm("recovery_threshold", Number(event.target.value))} /></label>
          </div>
