  ...
  return <div className="page-shell">
    ...
    {!loading && enabledWorkspaces.length === 0 ? <section className="panel file-governance-panel"><EmptyState title="还没有可用工作区" description="先登记并启用一个本机工作区，文件动作才有明确的操作边界。" /><Link className="button button-primary" to="/workspaces">去登记工作区 ↗</Link></section> : <section className="panel file-governance-panel">
      ...
      <form className="file-governance-form" onSubmit={(event) => void submit(event)}>
        <label htmlFor="file-workspace">工作区<select id="file-workspace" name="workspace_id" autoComplete="off" value={workspaceId} onChange={(event) => { setWorkspaceId(event.target.value); setEntryId(""); setResult(null); setFeedback(null); }} disabled={loading || submitting}><option value="">请选择工作区</option>{enabledWorkspaces.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
        <label htmlFor="file-action">动作类型<select id="file-action" name="action" autoComplete="off" value={kind} onChange={(event) => { setKind(event.target.value as FileActionKind); setResult(null); setFeedback(null); }} disabled={submitting}><option value="inspect">检查文件</option><option value="quarantine">隔离文件</option><option value="restore">恢复文件</option></select></label>
        {kind !== "restore" ? <label htmlFor="file-relative-path">相对路径<input id="file-relative-path" name="relative_path" value={relativePath} onChange={(event) => setRelativePath(event.target.value)} placeholder="例如：src/config.json" disabled={submitting} autoComplete="off" /></label> : <label htmlFor="file-quarantine-entry">待恢复文件<select id="file-quarantine-entry" name="quarantine_entry_id" autoComplete="off" value={entryId} onChange={(event) => setEntryId(event.target.value)} disabled={submitting || entries.length === 0}><option value="">{entries.length ? "请选择隔离记录" : "没有可恢复的隔离文件"}</option>{entries.map((entry) => <option value={entry.id} key={entry.id}>{entry.original_relative_path} · {entry.content_sha256.slice(0, 12)}…</option>)}</select></label>}
