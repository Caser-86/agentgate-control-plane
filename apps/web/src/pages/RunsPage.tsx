  ...
  return (
    <div className="page-shell">
      <div className="page-heading">
        <div><span className="eyebrow">01 / Operate</span><h1>Runs</h1><p>Observe agent intent before it becomes an external action.</p></div>
        <div className="heading-stat"><strong>{runs.length.toString().padStart(2, "0")}</strong><span>tracked runs</span></div>
      </div>
      <section className="composer panel">
        <div className="section-heading"><div><span className="eyebrow">New control run</span><h2>What should the agent investigate?</h2></div><span className="mono-note">mock / deterministic</span></div>
        <form onSubmit={submit}>
          <label htmlFor="task-request">Task request</label>
          <textarea id="task-request" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Describe the operational goal and its safety boundary…" rows={3} />
          <div className="composer-footer"><div className="example-chips" aria-label="Demo examples">{examples.map((example) => <button type="button" key={example} onClick={() => setPrompt(example)}>{example.startsWith("Rotate") ? "Rotate key → deny" : "Restore degraded API → approve"}</button>)}</div><button className="button button-primary" type="submit" disabled={submitting || prompt.trim().length < 5}>{submitting ? "Starting…" : "Start run"}<span aria-hidden="true">↗</span></button></div>
        </form>
      </section>
      {error && <p className="inline-error" role="alert">{error}</p>}
      <section className="runs-section panel">
        <div className="section-heading"><div><span className="eyebrow">Activity ledger</span><h2>Recent runs</h2></div><span className="mono-note">newest first</span></div>
        {loading ? <div className="loading-row">Loading run history…</div> : runs.length === 0 ? <EmptyState title="No runs in the ledger" description="Start with one of the deterministic demos above to see policy and approval events appear." /> : <div className="table-wrap"><table><thead><tr><th>Run</th><th>Request</th><th>Status</th><th>Provider</th><th>Steps</th><th>Updated</th></tr></thead><tbody>{runs.map((run) => <tr key={run.id}><td><Link className="run-link" to={`/runs/${run.id}`}>#{run.id.slice(0, 8)}</Link></td><td className="request-cell">{run.user_request}</td><td><StatusBadge value={run.status} /></td><td><code>{run.provider}</code></td><td>{run.step_count.toString().padStart(2, "0")}</td><td className="muted-cell">{formatTime(run.updated_at)}</td></tr>)}</tbody></table></div>}
      </section>
    </div>
  );
}
