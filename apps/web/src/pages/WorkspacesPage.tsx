...
export function WorkspacesPage() {
  ...
  const [form, setForm] = useState({ name: "", root_path: "C:\\AgentGate\\workspaces\\project", patterns: ".env,*.pem,*.key" });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({ kind: "success", message: "工作区已登记。请在该目录内准备文件后提交文件动作。" });
    } catch (error) {
  ...
  return <div className="page-shell">
    ...
    <section className="panel workspace-create-panel"><div className="section-heading"><div><span className="eyebrow">登记边界</span><h2>添加本机工作区</h2></div><span className="mono-note">路径会经过 API 校验</span></div><form className="workspace-form" onSubmit={(event) => void submit(event)}><label htmlFor="workspace-name">名称<input id="workspace-name" name="name" autoComplete="off" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="例如：源码工作区" required /></label><label htmlFor="workspace-root">Windows 根目录<input id="workspace-root" name="root_path" autoComplete="off" value={form.root_path} onChange={(event) => setForm({ ...form, root_path: event.target.value })} placeholder="C:\\AgentGate\\workspaces\\project" required /></label><label htmlFor="workspace-patterns">保护规则<input id="workspace-patterns" name="protected_patterns" autoComplete="off" value={form.patterns} onChange={(event) => setForm({ ...form, patterns: event.target.value })} placeholder=".env,*.pem,*.key" /></label><div className="workspace-form-footer"><span>建议保留 <code translate="no">.env</code>、<code translate="no">*.pem</code>、<code translate="no">*.key</code>。</span><button className="button button-primary" type="submit" disabled={saving}>{saving ? "正在登记…" : "登记工作区 ↗"}</button></div></form></section>
